...
do
  ...
  local function run_onTick(...)
    ...
    for i=1,32 do -- load composite input array and copy to output array for pass-through
	  ...
	  keyFlags = ArkLua.getKeyFlags and ArkLua.getKeyFlags()
		  or Ib
    
    if keyDown>0 and not keyFlags[keyDown] then
    ...
    if type(mc_onTick)=="function" then	
      local message = ("onTick()")
      local status, err = pcall(mc_onTick)
  ...
  local function run_onDraw(...)
    ...
    if type(mc_onDraw)=="function" then
      local message = ("onDraw()")
      local status, err = pcall(mc_onDraw)
