...
source={"ArkNetRadTurretx01d","repl.it/@mgmchenry"}

--[[

ArkNet Radar Control MC Inputs and Outputs
  Col1:
    GPS X :9
    GPS Y :10
    Alt Left :11
...
function main()
  ...
  function onTick()
    ...
    cz=1-clamp(abs(cx*cy)*2,0,1)
    if s1.h[4] or s2.h[4] then cz=0 end
    tc[1]=ts[2]*-12*cz-cx
    --tc[2]=ts[3]*-4*cz-cy
    tc[6]=clamp(tc[6]+(ts[3]*-4*cz-cy)/60,-1,1)
    tc[2]=tc[6]
...
function onDraw() 
  ...
  betterDrawRect(5,h-5,(w-9)*(ds.s+1)/2,2)
  
  --fov
  betterDrawLineRel(w-2,5,0,w-10)
  betterDrawRect(w-5,h-4,2, (w-9)*(tc[3])*-1 )
  ...
  if s1.h[4] or s2.h[4] then
    betterSetColor(cBlue)
    dStr(cx-cD,cy-cD-6,"RDR OFF")
  end
  cD = 5
  cx = wC - clamp(rX * rX * 64,-1,1) * w / 2
  cy = hC - clamp(rY * rY * 64,-1,1) * h / 2
  if rng>1 then
    dStr(cx-cD,cy+cD+2,floor(rng).." M")
  end
